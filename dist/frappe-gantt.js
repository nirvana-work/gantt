...
class Bar {
  ...
  prepare_values() {
    ...
    this.due_date_corner_radius = this.gantt.options.due_date_corner_radius;
    this.duration =
      date_utils.diff(this.task._end, this.task._start, 'hour') /
      this.gantt.options.step;
    this.due_date_duration =
      date_utils.diff(this.task._due_date_end, this.task._due_date_start, 'hour') /
      this.gantt.options.step;

    this.width = this.gantt.options.column_width * this.duration;
    this.due_date_width = this.gantt.options.column_width * this.due_date_duration;
  ...
  draw_due_date_bar() {
    if (this.invalid) return;
    this.$bar_due_date = createSVG('rect', {
      x: this.due_date_x,
      y: this.due_date_y,
      width: this.due_date_width,
      height: this.due_date_height,
      rx: this.due_date_corner_radius,
      ry: this.due_date_corner_radius,
      class: this.task.due_date_bar_class,
      append_to: this.bar_group
    });

    animateSVG(this.$bar_due_date, 'width', 0, this.due_date_width);
  }
  ...
  compute_due_date_x() {
    ...
    const diff = date_utils.diff(due_date_start, gantt_start, 'hour');
    let x = diff / step * column_width;

    if (this.gantt.view_is('Month')) {
      const diff = date_utils.diff(due_date_start, gantt_start, 'day');
      x = diff * column_width / 30;
    }
    return x;
  }
  ...
  setup_tasks(tasks) {
    ...
    this.tasks = tasks.map((task, i) => {
      ...
      if (task_end_values.slice(3).every(d => d === 0)) {
        ...
        task._due_date_end = date_utils.add(task._due_date_end, 24, 'hour');
      }
